...
def somatorio &termo
  (0..@n).map(&termo).reduce(:+)
end

def mysin x
  somatorio {|i| x ** (2 * i + 1) * (-1) ** i / factorial(2 * i + 2)}
end

def mycos x
  somatorio {|i| x ** (2 * i) * (-1) ** i / factorial(2 * i)}
end
...
def mypi
  somatorio {|k| 4.0 * (-1) ** k / (2.0 * k + 1.0)}
end

def mye
  somatorio {|k| 1.0 / factorial(k)}
end
